...
{
     public function HostJjs2ExpensesPage()
    {
        ...
        return view('host.jjs2.expenses', compact('host', 'expenses'));
    }


    public function HostJjs2ApprovedRequest($id)
    {
        if (!Auth::guard('hosts')->check()) {
            return redirect()->route('host.login.page')->with('error', 'Please log in.');
        }
    ...
    public function HostJjs2DeclineRequest($id)
    {
        if (!Auth::guard('hosts')->check()) {
            return redirect()->route('host.login.page')->with('error', 'Please log in.');
        }
        ...
        public function HostJjs2ExpensesPrintPage()
    {
        if (!Auth::guard('hosts')->check()) {
            return redirect()->route('host.login.page')->with('error', 'Please log in.');
        }
        ...
        return view('host.jjs2.print.expenses', compact('expenses'));
    }
}
