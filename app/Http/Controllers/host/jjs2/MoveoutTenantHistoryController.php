...
{
    public function Jjs2TenantHistoryPage()
    {
        if (!Auth::guard('hosts')->check()) {
            return redirect()->route('host.login.page')->with('error', 'Please log in.');
        }

        return view('host.jjs2.history');
    }

    public function Jjs2TenantBillingHistoryPage()
    {
        if (!Auth::guard('hosts')->check()) {
            return redirect()->route('host.login.page')->with('error', 'Please log in.');
        }
        ...
        $historyData = DB::table('history_billings')
            ...
            ->where('property_id', 3) // Filter where property_id = 1
            ->groupBy('tenant_code')
            ->get();

        return view('host.jjs2.tenants_billings_history', compact('historyData'));
    }

    public function Jjs2TenantPaymentHistoryPage()
    {
        if (!Auth::guard('hosts')->check()) {
            return redirect()->route('host.login.page')->with('error', 'Please log in.');
        }
        ...
        $paymentData = DB::table('history_payments')
            ...
            ->where('property_id', 3) // Filter by property ID
            ->groupBy('tenant_code')
            ->get();

        return view('host.jjs2.tenants_payments_history', compact('paymentData'));
    }

    public function Jjs2ViewTenantPaymentHistory($tenant_code)
    {
        if (!Auth::guard('hosts')->check()) {
            return redirect()->route('host.login.page')->with('error', 'Please log in.');
        }
        ...
        return view('host.jjs2.tenants_payments_details', compact('history_payments', 'tenant_code'));
    }


public function Jjs2ViewTenantBillingHistory($tenant_code)
{
    if (!Auth::guard('hosts')->check()) {
        return redirect()->route('host.login.page')->with('error', 'Please log in.');
    }
    ...
    // Pass the data to the view
    return view('host.jjs2.tenant_details', compact('history_billings', 'tenant_code'));
}
}
