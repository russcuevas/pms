...
{
    public function HostHubertExpensesPage()
    {
        ...
        return view('host.hubert.expenses', compact('host', 'expenses'));
    }


    public function HostHubertApprovedRequest($id)
    {
        if (!Auth::guard('hosts')->check()) {
            return redirect()->route('host.login.page')->with('error', 'Please log in.');
        }
    ...
    public function HostHubertDeclineRequest($id)
    {
        if (!Auth::guard('hosts')->check()) {
            return redirect()->route('host.login.page')->with('error', 'Please log in.');
        }
