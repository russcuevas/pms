...
{
    public function Jjs1TenantHistoryPage()
    {
        if (!Auth::guard('hosts')->check()) {
            return redirect()->route('host.login.page')->with('error', 'Please log in.');
        }

        return view('host.jjs1.history');
    }

    public function Jjs1TenantBillingHistoryPage()
    {
        if (!Auth::guard('hosts')->check()) {
            return redirect()->route('host.login.page')->with('error', 'Please log in.');
        }
        ...
        $historyData = DB::table('history_billings')
            ...
            ->where('property_id', 2) // Filter where property_id = 1
            ->groupBy('tenant_code')
            ->get();

        return view('host.jjs1.tenants_billings_history', compact('historyData'));
    }

    public function Jjs1TenantPaymentHistoryPage()
    {
        if (!Auth::guard('hosts')->check()) {
            return redirect()->route('host.login.page')->with('error', 'Please log in.');
        }
        ...
        $paymentData = DB::table('history_payments')
            ...
            ->where('property_id', 2) // Filter by property ID
            ->groupBy('tenant_code')
            ->get();

        return view('host.jjs1.tenants_payments_history', compact('paymentData'));
    }

    public function Jjs1ViewTenantPaymentHistory($tenant_code)
    {
        if (!Auth::guard('hosts')->check()) {
            return redirect()->route('host.login.page')->with('error', 'Please log in.');
        }
        ...
        return view('host.jjs1.tenants_payments_details', compact('history_payments', 'tenant_code'));
    }


public function Jjs1ViewTenantBillingHistory($tenant_code)
{
    if (!Auth::guard('hosts')->check()) {
        return redirect()->route('host.login.page')->with('error', 'Please log in.');
    }
    ...
    // Pass the data to the view
    return view('host.jjs1.tenant_details', compact('history_billings', 'tenant_code'));
}
}
