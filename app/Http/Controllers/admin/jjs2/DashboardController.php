...
public function AdminJjs2DashboardPage()
    {
        ...
        $totalCashOnHand = CashOnHand::where('property_id', 3)->sum('total_cash_amount');
        $totalCash = Payments::where('property_id', 3)
            ->where('mode_of_payment', 'cash')
            ->sum('amount');
        $totalOnline = Payments::where('property_id', 3)
            ->where('mode_of_payment', 'online payment')
            ->sum('amount');

        return view('admin.jjs2.dashboard', compact('totalCashOnHand', 'totalCash', 'totalOnline'));
    }

    public function AdminJjs2TurnOverMoney(Request $request)
    {
        $request->validate([
