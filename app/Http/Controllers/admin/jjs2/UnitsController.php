...
{
    public function AdminJjs2UnitsManagementPage()
    {
        ...
        return view('admin.jjs2.units_management', compact('units', 'vacantUnits'));
    }


    public function AdminJjs2TransferAndRepair(Request $request)
    {
        $request->validate([
    ...
    public function AdminJjs2MarkForRepair(Request $request)
    {
    ...
    public function AdminJjs2MarkAsRepaired(Request $request)
    {
        $request->validate(['unit_id' => 'required|integer|exists:units,id']);
...
public function AdminJjs2MoveOutTenant($unitId, $tenantId)
{
    $historyBillingIds = [];
    $historyPaymentIds = [];
    ...
    return redirect()->route('admin.jjs2.print.summary')->with('success', 'Tenant moved out successfully. You can now print the summary.');
}

    public function Jjs2printSummary()
    {
        return view('admin.Jjs2.print.print_summary');
    }

public function Jjs2printBillings()
{
    ...
    $pdf = PDF::loadView('admin.jjs2.print.print_billings', compact('billing'));
    return $pdf->download($fileName);
}


public function Jjs2printPayments()
{
    ...
    $pdf = PDF::loadView('admin.jjs2.print.print_payments', compact('payment'));
    return $pdf->download($fileName);
}

    public function AdminJjs2FollowUpBillings()
    {
    ...
    This is a friendly reminder from Admin Jjs2 Bldg that you have an outstanding balance of ₱" . number_format($billing->total_balance_to_pay, 2) . " for the billing month of {$billing->for_the_month_of}.
    ...
    Thank you,
    Admin Jjs2 Bldg", function ($message) use ($billing) {
