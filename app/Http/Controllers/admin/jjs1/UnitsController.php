...
{
     public function AdminJjs1UnitsManagementPage()
    {
        ...
        return view('admin.jjs1.units_management', compact('units', 'vacantUnits'));
    }


    public function AdminJjs1TransferAndRepair(Request $request)
    {
        $request->validate([
    ...
    public function AdminJjs1MarkForRepair(Request $request)
    {
    ...
    public function AdminJjs1MarkAsRepaired(Request $request)
    {
        $request->validate(['unit_id' => 'required|integer|exists:units,id']);
...
public function AdminJjs1MoveOutTenant($unitId, $tenantId)
{
    $historyBillingIds = [];
    $historyPaymentIds = [];
    ...
    {
        return view('admin.jjs1.print.print_summary');
    }

public function Jjs1printBillings()
{
    ...
    $pdf = PDF::loadView('admin.jjs1.print.print_billings', compact('billing'));
    return $pdf->download($fileName);
}


public function Jjs1printPayments()
{
    ...
    $pdf = PDF::loadView('admin.jjs1.print.print_payments', compact('payment'));
    return $pdf->download($fileName);
}

    public function AdminJjs1FollowUpBillings()
    {
    ...
    This is a friendly reminder from Admin Jjs1 Bldg that you have an outstanding balance of ₱" . number_format($billing->total_balance_to_pay, 2) . " for the billing month of {$billing->for_the_month_of}.
    ...
    Thank you,
    Admin Jjs1 Bldg", function ($message) use ($billing) {
