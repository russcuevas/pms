...
public function AdminJjs1DashboardPage()
    {
        ...
        $totalCashOnHand = CashOnHand::where('property_id', 2)->sum('total_cash_amount');
        $totalCash = Payments::where('property_id', 2)
            ->where('mode_of_payment', 'cash')
            ->sum('amount');
        $totalOnline = Payments::where('property_id', 2)
            ->where('mode_of_payment', 'online payment')
            ->sum('amount');

        return view('admin.jjs1.dashboard', compact('totalCashOnHand', 'totalCash', 'totalOnline'));
    }

    public function AdminJjs1TurnOverMoney(Request $request)
    {
        $request->validate([
