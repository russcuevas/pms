            ...
            <tbody>
                @foreach ($announcements as $announcement)
                    <tr>
                        ...
                        <td>
                            @if ($announcement->is_approved == 0)
                                <form action="{{ route('host.jjs2.announcements.approve', $announcement->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    <button class="btn btn-success btn-sm">Approve</button>
                                </form>
                                <form action="{{ route('host.jjs2.announcements.decline', $announcement->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    <button class="btn btn-danger btn-sm">Decline</button>
                                </form>
                            @else
                                <form action="{{ route('host.jjs2.announcements.delete', $announcement->id) }}" method="POST" style="display:inline;">
                                    @csrf
