                    ...
                    @foreach ($turnOvers as $index => $turnOver)
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                    <form action="{{ route('host.jjs2.turnovers.approve', $turnOver->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        <button type="submit" class="btn btn-success approve">Received</button>
                                    </form>

                                    <form action="{{ route('host.jjs2.turnovers.decline', $turnOver->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to decline this turnover?')">
                                        @csrf
                                        @method('DELETE')
