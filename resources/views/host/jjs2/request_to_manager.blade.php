...
<div class="container my-5">
    ...
    <table id="requestsTable" class="table table-bordered table-striped nowrap" style="width:100%">
        ...
        @foreach ($request_to_managers as $request)
            ...
            <td>
                @if (!$request->is_approved)
                    <form action="{{ route('host.jjs2.request_to_manager.approve', $request->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="btn btn-sm btn-success">Approve</button>
                    </form>
                    <form action="{{ route('host.jjs2.request_to_manager.decline', $request->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-sm btn-danger">Decline</button>
                    </form>
                @else
                    <form action="{{ route('host.jjs2.request_to_manager.delete', $request->id) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
